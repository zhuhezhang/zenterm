...
export default function TitleBar({ onOpenAbout }: { onOpenAbout?: () => void }) {
  ...
  useEffect(() => {
    const off = window.zenterm?.window.onMaximized((v) => setMaximized(v))
    window.zenterm?.window.isMaximized().then((res) => {
    ...
    <div className={`titlebar ${IS_MAC ? 'is-mac' : 'is-not-mac'}`}>
      ...
      {!IS_MAC && (
        <div className="titlebar-controls">
          <button className="titlebar-btn minimize" onClick={() => window.zenterm?.window.minimize()} title={t('titlebar.minimize')}>
            <svg width="10" height="1" viewBox="0 0 10 1"><rect width="10" height="1" fill="currentColor"/></svg>
          </button>
          <button className="titlebar-btn maximize" onClick={() => window.zenterm?.window.maximize()} title={maximized ? t('titlebar.restore') : t('titlebar.maximize')}>
            {maximized
          ...
          <button className="titlebar-btn close" onClick={() => window.zenterm?.window.close()} title={t('titlebar.close')}>
