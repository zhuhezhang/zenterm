...
简体中文 · **[English](README.md)** · v3.3.4

现有的远程终端很多功能都用不上，对于我来说不够简约，于是便有了此项目。ZenTerm 是一款基于 **Electron**、**React** 与 **xterm.js** 的跨平台桌面终端模拟器。支持 **SSH**、**SFTP**、**Telnet**、**串口（Serial）** 与 **本地（本机 Shell）** 连接，并提供会话保存、层级分组、加密凭据存储，以及无边框自定义界面（深色/浅色主题、中英双语）。
...
| **Local**  | 通过 `node-pty` 启动本机交互 Shell（Windows 为 ConPTY）；可选 Shell 路径与工作目录（空则系统默认 Shell / 用户主目录）；工作目录受同一套本地路径策略约束 |
...
- **退格键模式**（按会话）：自动（SSH/本地 发 DEL，Telnet/串口发 BS），或强制 DEL / BS
- **终端交互**：选中复制、右键粘贴（可在设置中关闭）
- **输出高亮**：正则规则着色（内置错误/成功/警告/IP 等默认规则）
- **标签栏**：新建连接、关闭当前/其他/左侧/右侧/全部、清屏、保存终端输出
- **会话日志**：关闭 / 开启（缓冲增量追加：与屏幕一致的纯文本，仅追加写入）；日志目录经路径策略校验
...
- 日志、SFTP 本地路径与 **本地 Shell 工作目录** 的 **本地路径策略**：用户主目录、文档、下载、桌面、音乐/图片/视频、userData；Windows 上另允许非系统盘根目录（如 `D:\`）
...
后端 electron/（handlers + lib）
    │  Worker 线程（SSH / SFTP）
    ▼
远程主机 / 本地串口 / 本机 PTY Shell / 系统钥匙串（safeStorage）
...
- **npm** 9+
- **原生模块编译环境**（Serial / 可选重编 node-pty 等需要）：
...
5. **路径沙箱**：会话日志、SFTP 本地路径与本地 Shell 工作目录须落在允许的用户目录、`userData` 或（Windows）非系统盘根目录内；拒绝时返回结构化 IPC 错误码，由渲染进程映射为 i18n 提示。
6. **串口安全**：仅允许连接 `listPorts` 枚举结果中的路径，降低任意设备打开风险。
7. **本地 Shell**：带路径分隔符的显式 Shell 路径须存在；裸命令名交由系统 `PATH` 解析。工作目录为空时使用用户主目录。
...
| 本地 Shell 失败 / `node-pty` 不可用                                                                                                                       | 确认已安装 `node-pty`（`prebuilds/` 下 N-API 预编译）；打包时会 `asarUnpack` 解压 `**/node_modules/node-pty/**`。必要时可执行 `npm run rebuild:native` 针对 Electron 重建                                                                                  |
| 本地 Shell 提示工作目录不允许                                                                                                                                  | 将工作目录选在主目录/文档/下载等允许根下；留空则使用用户主目录                                                                                                                                                                                          |
...
| 报错 `No handler registered for 'local:connect'`                                                                                                        | 重新执行 `npm run dev`，确保主进程在 `build:main` 完成后再启动（开发脚本现已先编译主进程再拉起 Electron）                                                                                                                                                        |
